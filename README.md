...
*(Comparison of the linearized semi-empirical drag model fittings against experimental and numerical datasets. The experimental measurements are fitted using a Weighted Least Squares (WLS) regression accounting for the indicated biaxial uncertainties ($\pm\delta x,\ \pm\delta C_d$), with dashed lines representing the bounding envelope derived via the end-points pivot rule. Computational (CFD) results utilizing the SST $k$--$\omega$ turbulence model are fitted using an Ordinary Least Squares (OLS) approach.)*
